      ...
      label = "check the values inserted by the user make sense, if so move to the next info; or show error",
      {
        ...
        if (
          ...
          numeric_check <- all(sapply(
            rv_local$loaded_data[, expected_numeric, drop = FALSE],
            function(x) {
              suppressWarnings(
          ...
          character_check <- all(sapply(
            rv_local$loaded_data[, expected_character],
            function(x) {
          ...
          if (numeric_check && character_check) {
            ...
            df_selected <- data.frame(
              row.names = seq_len(nrow(rv_local$loaded_data))
            )
            ...
            if (!numeric_check) {
              error_txt <- c(
                error_txt,
                "Some numeric values are not in the format we expected"
              )
            }
            ...
            if (!character_check) {
              error_txt <- c(
                error_txt,
                "Some character values are not in the format we expected"
              )
            }

            output$format_error <- renderUI({
              return(p(
                paste(error_txt, collapse = " & "),
                style = "color: red;"
              ))
            })
          }
          ...
          if (numeric_check) {
            ...
            if (any(lons < -8 | lons > 2 | lats < 49 | lats > 61)) {
              output$format_error_date_and_time <- renderUI({
                return(p(
                  "The coordinates you provided are not in the UK, please check your data",
                  style = "color: red;"
                ))
              })
              return()
            ...
            if (input$time_type == "split_date") {
              ...
              if (!is.na(the_columns[["month"]])) {
                if (
                  !any(
                    is.na(as.numeric(as.character(the_columns[["month"]]))) &
                      !is.na(the_columns[["month"]])
                  )
                ) {
              ...
              if (!is.na(the_columns[["day"]])) {
                if (
                  !any(
                    is.na(as.numeric(as.character(the_columns[["day"]]))) &
                      !is.na(the_columns[["day"]])
                  )
                ) {
              ...
              dates <- get_right_date_format(
                rv_local$loaded_data[[the_columns[["date"]]]],
      ...
      label = "check privacy, if so load onto db",
      {
        if (!input$acceptCheckbox) {
          output$privacy_text <- renderUI({
            return(p(
              "You must accept to upload your data",
              style = "color: red;"
            ))
          })
        } else {
