  ...
  tagList(
    ...
    div(style = "display: flex; justify-content: space-between; align-items: center; width: 100%;",
      ...
      div(style = "flex: 1; text-align: left;",
          actionButton(inputId = ns("back_upload"), label = div(icon("chevron-left", lib = "font-awesome"), "back", width = "30%"), style = "font-size:100%;", class = "btn-danger")
          ),
      # Other three download buttons aligned to the left
      div(style = "flex: 3; display: flex; justify-content: flex-end; gap: 2px;",
          downloadButton(ns("download_documentation"), "Documentation", class = "btn-info", style = "font-size:100%; display: inline-block;"),
          downloadButton(ns("download_report"), "Report", class = "btn-danger", style = "font-size:100%; display: inline-block;"),
          downloadButton(ns("download_rawdata"), "Raw Data", class = "btn-danger", style = "font-size:100%; display: inline-block;")
          )
      ),
        ...
        fluidRow(
          column(6,
                 bslib::navset_card_underline(
                   ...
                   bslib::nav_panel("Guide",
                                    shiny::img(src="www/example_guide.png", align = "center", style = "max-width: 100%;")
                                    )
                   )
                 ),
          column(6,
                 bslib::navset_card_underline(
                   height = 500,
                   bslib::nav_panel("Uncertainty Graph",
  ...
  moduleServer(id, session = x, function(input, output, session){
    ...
    output$uncertainty_graph <- renderPlot({
      ...
      if (isTruthy(rv$data_results)){
        ...
        uncertainty_df <- data.frame(x = x_val,
                                     y = dnorm(x_val, 0, 1))
    ...
    output$map_result <- leaflet::renderLeaflet ({
      ...
      if (isTruthy(rv$data_results)){
        ...
        #to coordinates
        data <- dplyr::filter(rv$data_results,
                              z == 0.55) %>%
        ...
        leaflet::leaflet() %>%
          ...
          leaflegend::addSymbolsSize(data = data,
                                     values = ~S_cz, #size
                                     shape = 'circle', color = 'black',fillColor = ~numPal(S_cz), #color
                                     opacity = .5, baseSize = 10) %>%
          leaflegend::addLegendSize(values = data$S_cz,
                                    color = 'black',
                                    title = 'S_cz_2004',
                                    shape = 'circle',
                                    orientation = 'horizontal', opacity = .5, fillOpacity = 0,
                                    breaks = 5,
                                    position = 'bottomright') %>%
          leaflegend::addLegendNumeric(pal = numPal,
                                       title = 'S_cz_2004',
                                       shape = 'stadium',
                                       values = data$S_cz,
                                       fillOpacity = .5,
    ...
    output$download_report <- downloadHandler(
      filename = function() {
        paste0("soccatoa_report", "_", stringr::str_replace_all(Sys.Date(), "-", "_"), ".html")
        # paste0("soccatoa_report", "_", stringr::str_replace_all(Sys.Date(), "-", "_"), ".pdf")
      },
      content = function(file) {

        tempReport <- "/data/notebooks/rstudio-madtigsoccatoa/soccatoa/inst/app/www/downloadables/example_report.Rmd"
